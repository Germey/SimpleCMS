        ...
        <div class="list-box">
            <ul class="simple">
                <?php if( $category['id'] ) { $child=D('Category')->getChildrenId($category['id'],true); }else { $child=D('Category')->getField('id',true); };if(false)$filter['weight']=false;if($_GET['stid'])$filter['tag_id']=$_GET['stid'];if(true)$count = D('Content')->getPagesByTypeId($child, 0, 0,$filter, true);if(true)list($pagesize, $page_num, $pagestring) = pagestring($count, 20);if(true)$pages = D('Content')->getPagesByTypeId($child, $page_num, $pagesize,$filter,false,false,true);else $pages = D('Content')->getPagesByTypeId($child, 1, 20,$filter,false,false,true,false);if(false)var_dump($pages); if(is_array($pages)): $i = 0; $__LIST__ = $pages;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$one): $mod = ($i % 2 );++$i;?><li>
                        ...
                        <?php if($one['tags']): ?>&nbsp;&nbsp;&nbsp;<i title="标签" class="icon icon-tag"></i>
                            <?php if(is_array($one['tags'])): foreach($one['tags'] as $key=>$t): ?><a style="color: #3a87ad" href="/category/<?php echo ($one['category_id']); ?>?stid=<?php echo ($t['tag_id']); ?>"><?php echo ($t['name']); ?></a>&nbsp;&nbsp;<?php endforeach; endif; endif; ?>
                        <span class="pull-right muted"><small><?php echo substr($one['publish_time'],0,10);?></small></span>
<!--                         <?php if($one['summary']): ?><blockquote><?php echo get_short($one['summary'],170);?></blockquote><?php endif; ?> -->
 ...
 $childs = D('Category')->getChildren($category['parent']?$category['parent']['id']:$category['id']); ?>
    <?php if(is_array($childs)): foreach($childs as $key=>$one): ?><a href="/category/<?php echo ($one['id']); ?>">
        ...
        <?php if(($category['parent']['id'] == 45) OR ($category['parent']['id'] == 53)): $links = D('Banner')->getBannerByName('quick_links'); ?>
<div class="list-box">
    <div class="list-box-head">资源链接</div>
    <ul class="simple">
        <?php if(is_array($links)): foreach($links as $key=>$one): ?><li>
                <a href="<?php echo ($one['link']); ?>" target="_blank"><?php echo ($one['title']); ?></a>
