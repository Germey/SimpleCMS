...
<div id="page-container" class="sidebar-partial sidebar-visible-lg sidebar-no-animations">
    ...
    <div class="block">
        <?php if($configs): ?><form action="<?php echo U('config/save');?>" method="post" class="form-horizontal post_ajax_form">
              <table class="table table-noborder">
                <tr><th style="text-align:right">题目</th><th style="width:550px">内容</th><th>读取关键字<span class="text-danger">（不能为空，请勿重复）</span></th></tr>
                  <?php if(is_array($configs)): foreach($configs as $key=>$one): ?><tr>
                        <td class="item-label"><?php echo ($one['title']); ?></td>
                        <td>
                            <?php if($one['edit_type'] == 'textarea'): ?><textarea name="value<?php echo ($one['id']); ?>" type="text" rows="5" class="pull-left form-control input-sm"><?php echo ($one['value']); ?></textarea>
                            <?php else: ?>
                              <input name="value<?php echo ($one['id']); ?>" type="text" class="pull-left form-control input-sm" value="<?php echo ($one['value']); ?>" /><?php endif; ?>
                            <div class="clearfix"></div>
                            <div class="text-muted"><?php echo ($one['remark']); ?></div>
                        </td>
                        <td><input name="key<?php echo ($one['id']); ?>" type="text" class="pull-left form-control input-sm" style="width:150px;" value="<?php echo ($one['key']); ?>" />
                        <?php if($type == 'custom'): ?>&nbsp;&nbsp;<a href="<?php echo U('Config/delete?id='.$one['id']);?>" class="text-danger ajaxlink" ask="确定删除【<?php echo ($one['title']); ?>】配置？"><i class="hi hi-remove"></i>删除</a><?php endif; ?>
                        </td>
              ...
              <div class="form-group form-actions">
                  ...
                  <input type="hidden" name="config_ids" value="<?php echo implode(',', get_column($configs));?>" />
        ...
        <br />
        <?php if($type == 'custom'): ?><p><a href="<?php echo U('Config/render_new');?>" class="btn btn-default ajaxlink"><i class="fa fa-plus"></i>增加配置项</a></p><?php endif; ?>
    </div>
</div>
