                    ...
                    <tr>
                        <td>
                            <div class="btn-group btn-group-sm btn-create-group">
                                  <a href="<?php echo U('render_edit?manager_role=editor');?>" class="btn btn-primary ajaxlink">新增</a>
                                  <button type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown">
                                    <span class="caret"></span>
                                  </button>
                                  <ul class="dropdown-menu" role="menu" style="width:110px;">
                                    <?php if(is_array($manager_role)): foreach($manager_role as $k=>$one): ?><li><a class="ajaxlink" href="<?php echo U('render_edit?manager_role='.$one[type]);?>"><i class="<?php echo ($one['icon_class']); ?>" ></i><?php echo ($one['name']); ?></a></li><?php endforeach; endif; ?>
                                  </ul>
                            </div>
                        </td>
                        <td>
                            <div class="input-group">
                              <input class="form-control input-sm" name="susername" value="<?php echo ($susername); ?>" placeholder="用户名" />
    ...
    <div class="block">
        <table id="general-table" class="table table-striped table-vcenter table-hover">
            ...
            <tbody>
                ...
                <?php if(is_array($user_list)): foreach($user_list as $key=>$one): ?><tr <?php if($hilight_id == $one['id']): ?>class="success"<?php endif; ?>  <?php if($login_user['uid'] == $one['id']): ?>style="font-weight:bold"<?php endif; ?> >
                        <td nowrap><?php echo ($one['username']); ?></td>
                        <td nowrap><?php echo ($one['email']); ?></td>
                        <td nowrap><?php echo ($one['type_display_name']); ?></td>
                        <td>
                            <?php if($one['role'] == 'manager'): echo ($one['category_display']); ?>
                            <?php else: ?>
                                <?php echo implode(' , ',$one['category_display']);?></td><?php endif; ?>
                        <td style="min-width:100px">
                            <?php if(($login_user['uid'] > 10) AND ($one['id'] < 10)): ?>-
                            <?php else: ?>
                                <div class="btn-group btn-group-xs">
                                    <?php if($login_user['role'] == 'manager'): ?><a href="<?php echo U('render_edit?manager_role='.$one['manager_role']).'/user_id/'.$one['id'];?>" title="编辑栏目权限" class="btn btn-default ajaxlink"><i class="hi hi-pencil"></i></a>
                                        <a href="<?php echo U('changePassword?user_id='.$one['id']);?>" title="修改密码" class="btn btn-default ajaxlink" ><i class="gi gi-lock"></i></a>
                                        <?php if($login_user['uid'] != $one['id']): ?><a href="<?php echo U('User/delete?id='.$one['id']);?>" title="删除" class="btn btn-danger ajaxlink" 
                                                 ask="确认要删除该用户?" >
                                                <i class="<?php echo ($one_content['status']<10?'hi hi-trash':'hi hi-remove'); ?>"></i>
                                            </a><?php endif; ?>
                                    <?php else: ?>
                                        <?php if($login_user['uid'] == $one['id']): ?><a href="<?php echo U('changePassword?user_id='.$one['id']);?>" title="修改密码" class="btn btn-default ajaxlink" ><i class="gi gi-lock"></i></a><?php endif; endif; ?>
                                </div><?php endif; ?>
                        </td>
