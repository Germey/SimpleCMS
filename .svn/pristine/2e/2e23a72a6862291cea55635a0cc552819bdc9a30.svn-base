...
class FieldModel extends Model {
    ...
    public function getContentFields($model_id, $category_id) {
        ...
        if($category['override_fields']) {
            ...
            foreach($a as $v) {
                $override = explode(':',$v);
                $key = $override[0];
                if(($fields[$key] || strpos($key, 'inner_extend_')===0) && count($override)>1) {
                    $fields[$key]['title'] = $override[1];
                    $fields[$key]['placeholder'] = $override[2];
                    $fields[$key]['type'] = $override[3];
                    // 简单处理一下select
                    if($fields[$key]['type']=='select') {
                        $fields[$key]['options'] = explode(',', $override[4]);
                    }
                }
            }
        }
    ...
    public function getCategoryFields($model_id, $category_id) {
        ...
        $fields = array(
            ...
            "override_fields" => array("title" => "字段覆盖显示", "placeholder" => "field_name:new_display_name:new_placeholder;summary:新的摘要:这个摘要是做这个用途的;inner_extend_1:类型:类型的解释:select:A,B,C"),
