...
function simple_down_xls($data, $keynames, $name='data-export') {
    ...
    $xls[] = '</table></body></html>';
    $xls = join("\r\n", $xls);
    $name = iconv("UTF-8","GB2312//IGNORE",$name . '-' . date('Ymdhis'));
    header('Content-Disposition: attachment; filename="'.$name.'.xls"; charset="gb2312"');
...
function get_string_encoding($str){
    ...
    $encoding=mb_detect_encoding($str, array('GBK', 'GB2312','EUC-CN','UCS-2','UTF-16','BIG5','ASCII'));
    if(empty($encoding)){
